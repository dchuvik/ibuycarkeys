...
export const POST: APIRoute = async ({ cookies, request }) => {
	...
	if (!id || !validStatuses.has(status)) {
		return new Response(JSON.stringify({ error: "Invalid contact id or status", ok: false }), {
			status: 400,
			headers: { "Content-Type": "application/json" },
		});
	}
	...
	if (error) {
		return new Response(JSON.stringify({ error: "Could not update contact status", ok: false }), {
			status: 500,
			headers: { "Content-Type": "application/json" },
		});
	}
	...
	if (!data?.length) {
		return new Response(JSON.stringify({ error: "Contact message not found", ok: false }), {
			status: 404,
			headers: { "Content-Type": "application/json" },
		});
	}
